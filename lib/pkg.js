...
function flattenDeps(deps = [], original = {}) {
  const flat = {
    ...
    ...Object.fromEntries(
      deps
        .map((pkg) => pkg.version.pending && [pkg.name, '^' + pkg.version.pending])
        .filter(Boolean)
    ),
  }
...
class Pkg {
  ...
  buildDepList(graph) {
    ...
    this.peerDependencies =
      peerDependencies && Object.keys(peerDependencies).map(link).filter(Boolean)
  }
  ...
  needsMajorUpdateIfConsumedFlat(checkPeers = true) {
    const majorDeps = this.dependencies?.some(
      (dep) => !semver.satisfies(dep.version.pending, this._contents.dependencies[dep.name])
    )
    const majorPeer =
      checkPeers &&
      this.peerDependencies?.some(
        (dep) => !semver.satisfies(dep.version.pending, this._contents.peerDependencies[dep.name])
      )
